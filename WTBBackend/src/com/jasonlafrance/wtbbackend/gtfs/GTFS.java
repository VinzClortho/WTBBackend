...
public final class GTFS {
    ...
    private static final int WINDOW_MARGIN = Config.get().getIntOption(Config.STOP_WINDOW_MARGIN);
    ...
    private static final StackedStopList sMasterSSL = StackedStopList.getInstance();
    ...
    // 
    private static void updateMasterStopWindow(ArrayList<ArrayList<StopAdapter>> inStops, int inID) {
    ...
    private static void updateMasterStackedStopList() {
        ...
        synchronized (sStopsMap) {
            ...
            for (int idKey : ids) {
                ArrayList<ArrayList<StopAdapter>> nowStops = sStopsMap.get(idKey);
                if (nowStops != null) {
    ...
    /*
     public static Agency getAgency(String key){
     for(int i = 0; i < mAgencies.length; i++){
     if(mAgencies[i].get_agency_id().equals(key)){
     return mAgencies[i];
     }
     }
     return null;
        ...
        synchronized (sStopsMap) {
            for (int id = 0; id < _nextID; id++) {
                ...
                if (sList != null) {
                    ...
                    while (outer.hasNext()) {
                        ArrayList<Stop> sFragment = (ArrayList<Stop>) outer.next();
    ...
    private final SimpleDateFormat _dateFormatter = new SimpleDateFormat("yyyyMMdd");
    ...
    private final double _thresholdMeters = Config.get().getDoubleOption(Config.CLOSENESS_THRESHOLD);
    // this is the compiled master path list!

    private ArrayList<RoutePath> _paths;

    private double _minLat = Double.MAX_VALUE, _maxLat = -Double.MAX_VALUE;
    private double _minLon = Double.MAX_VALUE, _maxLon = -Double.MAX_VALUE;
    ...
    public GTFS(String inDir, int inTimeGap) throws Exception {
        ...
        for (String table : _gtfsClasses) {
            ...
            try {
                ...
                //Constructor<?> cons = GTFSParser.getConstructor(table, false);
                ...
                if (line != null) {
                    ...
                    while (line != null && line.trim().length() > 0) {
                        ...
                        //GTFSParser e = (GTFSParser) cons.newInstance(line.trim(), _id);
                        ...
                        if (e.getID() != 0) {
                            ...
                            Object o = _classListMap.get(e.getClass().getName());
                            if (o != null) {
                ...
                System.out.println(table + ": " + _classListMap.get(table).size());
        ...
        for (int i = 0; i < mRoutes.size(); i++) {
            ...
            if (mAgencies.size() == 1) {
                ...
                for (int a = 0; a < mAgencies.size(); a++) {
                    if (mAgencies.get(a).get_agency_id().equals(e.get_agency_id())) {
    ...
    private synchronized void updateStopWindow() {
        ...
        ArrayList<ArrayList<StopAdapter>> nowStops = getAllStopsInTimeframe(
                minutesToTime(now - (WINDOW_MARGIN * 2)), // hysterisis for the window margins
            ...
            Collections.sort(mVertices,
                    new Comparator<Vertex>() {
                        public int compare(Vertex a, Vertex b) {
                            int ret = a.get_shape_id().compareToIgnoreCase(b.get_shape_id());
                            if (ret == 0) {
                                if (a.get_shape_pt_sequence() < b.get_shape_pt_sequence()) {
                                    ret = -1;
                                } else if (a.get_shape_pt_sequence() > b.get_shape_pt_sequence()) {
                                    ret = 1;
                                }
                            }
                            return ret;
                    ...
                    new Comparator<Trip>() {
                        ...
                        public int compare(Trip a, Trip b) {
                            if (a.get_shape_id() == null && b.get_shape_id() != null) {
                                return 1;
                            } else if (b.get_shape_id() == null && a.get_shape_id() != null) {
                                return -1;
                            } else if (a.get_shape_id() == null && a.get_shape_id() == null) {
                                return 0;
                            }
                            return a.get_shape_id().compareToIgnoreCase(b.get_shape_id());
                        }
                    }
            );
            isSorted = true;
    ...
    private void addStopsToTrips() {
        ...
        for (int s = 0; s < mStopTimes.size(); s++) {
            ...
            if (_validServices.contains(_tripMap.get(st.get_trip_id()).get_service_id())) {

                // clone the stop...
                StopAdapter stop = new StopAdapter(_stopMap.get(st.get_stop_id()));
    ...
    private void buildRoutePaths() {
        ...
        Collections.sort(paths,
                new Comparator<LinkedList<Trip>>() {
                    @Override
        ...
        /*
         int i = 1;
         for (LinkedList<Trip> e : paths) {
         if (e != null) {
         String name = e.get(0).getRoute().get_route_short_name();
         name = name.replace('/', '-');
         KMLUtil.KMLExport.writeKML(name + "_" + i + ".kml", e);
         }
         i++;
         }
         */
    ...
    private ArrayList<RoutePath> attachTrips(LinkedList<LinkedList<Trip>> paths, int timeThreshold, boolean matchStops) {
        ...
        do {
            ...
            while (!paths.isEmpty()) {
                // grab the first one
                piece = paths.pop();
                while (piece.getFirst().getVertices().isEmpty() && !paths.isEmpty()) {
                    piece = paths.pop();
                }
                ...
                while (!paths.isEmpty() && li.hasNext()) {
                    ...
                    if (matchStops) {
                        leftStop = t.getLast().getEndStopID() == piece.getFirst().getStartStopID()
                                || t.getLast().getVertices().get(t.getLast().getVertices().size() - 1).getRawDistanceInMeters(piece.getFirst().getVertices().get(0)) <= rawThresh;
                        rightStop = t.getFirst().getStartStopID() == piece.getLast().getEndStopID()
                                || t.getFirst().getVertices().get(0).getRawDistanceInMeters(piece.getLast().getVertices().get(piece.getLast().getVertices().size() - 1)) <= rawThresh;
                    }
                    ...
                    if (leftStop
                            && (t.getLast().get_service_id() == null ? piece.getFirst().get_service_id() == null : t.getLast().get_service_id().equals(piece.getFirst().get_service_id()))
                            && (t.getLast().get_route_id() == null ? piece.getFirst().get_route_id() == null : t.getLast().get_route_id().equals(piece.getFirst().get_route_id()))) {
                        piece.addAll(0, t);
                        addedTo = true;
                        li.remove();
                        foundMatch = true;
                        // or check of t fits after piece
                            ...
                            && (t.getFirst().get_service_id() == null ? piece.getLast().get_service_id() == null : t.getFirst().get_service_id().equals(piece.getLast().get_service_id()))
                            && (t.getFirst().get_route_id() == null ? piece.getLast().get_route_id() == null : t.getFirst().get_route_id().equals(piece.getLast().get_route_id()))) {
                        piece.addAll(t);
                        addedTo = true;
                        li.remove();
                        foundMatch = true;
        ...
        ListIterator li = paths.listIterator();
        while (li.hasNext()) {
            ArrayList<Trip> tl = new ArrayList<Trip>((LinkedList<Trip>) li.next());
            outPaths.add(
                    new RoutePath(tl, tl.get(0).getRoute())
            );
        }
    ...
    private void linkStructure() {
        ...
        /*
         // check for current route_paths.txt file
         FileInputStream fin = null;
         InputStreamReader isr = null;
         BufferedReader br = null;
         // load in the table
         boolean routePathExists = false;
         try {
         fin = new FileInputStream(_Dir + "\\route_path.txt");
         routePathExists = true;
         ...
         if (routePathExists) {
         _paths = new ArrayList<>();
         System.out.println("route_path.txt found.  Using...");
         try {
         isr = new InputStreamReader(fin, "UTF-8");
         br = new BufferedReader(isr);
         ...
         while (line != null) {
         RoutePath rp = new RoutePath(line, _tripMap);
         if (_validServices.contains(rp.getServiceID())) {
         _paths.add(rp);
         }
         line = br.readLine();
        ...
        /*
         ...
         try {
         writer = new BufferedWriter(new FileWriter(_Dir + "\\route_path.txt"));
         for (RoutePath rp : _paths) {
         writer.write(rp.getCSVTripList());
         }
         } catch (IOException e) {
         } finally {
         try {
    ...
    private void showRoutePaths() {
        for (RoutePath rp : _paths) {
            ...
            System.out.println("<-- (" + rp.getPath().get(rp.getPath().size() - 1).getLastStop().getStop().getID() + ") " + minutesToTime(rp.getPath().get(rp.getPath().size() - 1).getEndTime()));
        }
    }
            ...
            if (li.hasNext()) {
                ...
                while (li.hasNext()) {
                    ...
                    if (keep.getRouteID() == check.getRouteID()
                            && (keep.getServiceID() == null ? check.getServiceID() == null : keep.getServiceID().equals(check.getServiceID()))
                            && keep.getStartTimecode() == check.getStartTimecode()
    ...
    public ArrayList<ArrayList<Vertex>> getAllVerticesInTimeframe(String inStart, String inEnd) {
        ArrayList<ArrayList<Vertex>> output = new ArrayList<>();

        for (Route r : mRoutes) {
            ArrayList<ArrayList<Vertex>> list = getVerticesInTimeframe(r.getName(), inStart, inEnd);
            output.addAll(list);
        }
    ...
    /*
    public ArrayList<RouteEdges> getAllRouteEdgesInTimeframe(String inStart, String inEnd) {
        ArrayList<RouteEdges> output = new ArrayList<>();
...
*/
     ...
     *
     * @param inRoute String of routes short name
     * @param inStart String of start time in the format HH:MM:SS
     * @param inEnd String of end time in the format HH:MM:SS
     * @return A nested ArrayList of Vertex describing the path in range
     */
    public ArrayList<ArrayList<Vertex>> getVerticesInTimeframe(String inRoute, String inStart, String inEnd) {
            ...
            if (checkPath.getRouteName().equals(inRoute)
                ...
                if (cStart <= iEnd && cEnd >= iStart) {
                    ...
                    while (ti.hasNext() && !done) {
                        ...
                        ListIterator vi = checkTrip.getVertices().listIterator();
                        while (vi.hasNext() && !done) {
                            Vertex v = (Vertex) vi.next();
                            if (v.isStop()) {
                                int vTime = v.getStop().getStopTime().getArrivalTimecode();
                                //System.out.println(v.getStop().getStopTime().get_arrival_time() + " -> " + v.getStop().getStopTime().getArrivalTimecode());
                                if (!inRange && vTime >= iStart) {
                                    inRange = true;
                                } else if (inRange && vTime != 0 && vTime > iEnd) {
                                    inRange = false;
                                    done = true;
                                }
                            }
    ...
    public ArrayList<ArrayList<StopAdapter>> getAllStopsInTimeframe(String inStart, String inEnd) {
        ArrayList<ArrayList<StopAdapter>> output = new ArrayList<>();

        for (Route r : mRoutes) {
            //System.out.println(r.getName());
            ArrayList<ArrayList<StopAdapter>> list = getStopsInTimeframe(r.getName(), inStart, inEnd);
            output.addAll(list);
        }
    ...
    public ArrayList<ArrayList<StopAdapter>> getStopsInTimeframe(String inRoute, String inStart, String inEnd) {
        ArrayList<ArrayList<Trip>> foundPoints = new ArrayList<>();
        ArrayList<ArrayList<StopAdapter>> output = new ArrayList<>();
        ...
        for (int i = 0; i < _paths.size(); i++) {
            ...
            // found a matching route
            // now find its relative points
            if (checkPath.getRouteName() != null && checkPath.getRouteName().equals(inRoute)
                    && _validServices.contains(checkPath.getServiceID())) {
                // check if there are points within the time frame
                        ...
                        while (si.hasNext() && !done) {
                            ...
                            if (!inRange && sTime >= iStart) {
                                ...
                                while (si.hasPrevious()) {
                                    StopAdapter sc = (StopAdapter) si.previous();
